const AddBook = {
  ...
  <h2 v-if="error" style="text-align: center; color: red;">{{ error }}</h2>
    <div v-if="!error" class="centered-form" style="margin-top: 10px;">
  ...
  methods: {
    ...
    async fetchSections() {
      try {
        ...
        if (res.status === 403) {
          this.error = 'You are not authorized to fetch sections.'
          return
        }
    ...
    async submitForm() {
      ...
      try {
        ...
        if (res.status === 403) {
          this.error = 'You are not authorized to add this book.'
          return
        }
